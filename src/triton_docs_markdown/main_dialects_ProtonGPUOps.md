...
_Initialize the intra kernel profiler_
...
Initialize the intra kernel profiler by filling the auxiliary metadata to the header. `scratchPtr` is the base address of the profiling scratch buffer where the header is stored.
...
Operand | Description  
---|---  
`buffer` | memory descriptor type (`::mlir::triton::gpu::MemDescType`) in Triton IR type system or ptr  
