...
triton.language.atomic_xchg(_pointer_ , _val_ , _mask =None_, _sem =None_, _scope =None_, __semantic =None_)¶
...
This function can also be called as a member function on [`tensor`](triton.language.tensor.html#triton.language.tensor "triton.language.tensor"), as `x.atomic_xchg(...)` instead of `atomic_xchg(x, ...)`.
