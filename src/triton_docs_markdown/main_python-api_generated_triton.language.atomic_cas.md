...
Parameters:
  ...
  * **cmp** (_Block_ _of_ _dtype=pointer.dtype.element_ty_) – The values expected to be found in the atomic object
...
This function can also be called as a member function on [`tensor`](triton.language.tensor.html#triton.language.tensor "triton.language.tensor"), as `x.atomic_cas(...)` instead of `atomic_cas(x, ...)`.
