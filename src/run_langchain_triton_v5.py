...
# 严格按要求 import，不修改 system prompt
from memories.TreeCheatsheetManager import TreeCheatsheetManager
...
DEFAULT_PATH = "triton_tree_cheatsheet_v2.json"
OUTPUT_DIR = "../outputs/triton_langchain_tree_cheatsheet"
...
class ThreadSafeCheatsheetManager:
    """包装 tree cheatsheet manager，添加线程锁以支持并发操作"""
    def __init__(self, tmp_path, default_path):
        ...
        with open(default_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.manager = TreeCheatsheetManager(initial_state=data)
    ...
    def build_reflection_update_prompt(
        ...
        with self.lock:
            return self.manager.build_reflection_update_prompt(
                question=question,
    ...
    def get_last_retrieval_context(self):
        with self.lock:
            return self.manager.data.get("metadata", {}).get("last_retrieval_context")
    ...
    def prune_by_utility(self, min_usage_ratio):
        with self.lock:
            self.manager.prune(
                {
                    "prune_threshold": -float(min_usage_ratio),
                }
            )

    def integrate_reflection_items(self, reflection_payload: str):
        with self.lock:
            return self.manager.integrate_reflection(
                reflection_text=reflection_payload,
                extractor=lambda text: json.loads(text),
                embedder=None,
                summarizer=None,
            )
...
def create_triton_tools(dataset, safe_manager: ThreadSafeCheatsheetManager, curation_model_name: str):
    ...
    def curate_cheatsheet(curation_context: str):
        ...
        try:
            ...
            prompt = safe_manager.build_reflection_update_prompt(
                question=question,
                model_answer=model_answer,
                model_reflection=model_reflection,
                retrieved_context=safe_manager.get_last_retrieval_context(),
            )
        except Exception as e:
            return f"⚠️ 知识库更新失败: 无法解析 curation_context: {e}"

        try:
            llm_response = curation_llm.invoke(prompt)
            response_text = llm_response.content if hasattr(llm_response, "content") else str(llm_response)
            safe_manager.integrate_reflection_items(response_text)
