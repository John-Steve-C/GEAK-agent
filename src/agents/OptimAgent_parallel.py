...
class OptimAgent(Reflexion_Oneshot):
    ...
    def run(self, output_path=None, multi_thread=True, thread_num=128, datalen=None, iteration_num=0, temperature=0, ancestor_num=2, start_idx=0, gpu_id=0, start_iter=0):
        """
        Args:
    ...
    def generate_dc(self, mem, method, temperature):
        with self.cheatsheet_lock:
            if method == "json":
                ...
                text = self.cheatsheet_manager.build_prompt(mem.ps.instruction, mem.raw_code[0])
            else:
