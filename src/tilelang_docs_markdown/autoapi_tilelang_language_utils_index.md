# tilelang.language.utilsÂ¶

Utils in TileLang operators.

## FunctionsÂ¶

`region`(buffer, access_type, *args) | Create a tl.region call for a BufferLoad and extents.  
---|---  
`buffer_region_to_tile_region`(buffer_region, ...) | Clamp extents and return a tl.region call.  
`index_to_coordinates`(index, shape) | Convert a flat (linear) index into multi-dimensional coordinates for a given shape.  
`linear_index`(*args) | Compute a flat (linear) index from multi-dimensional coordinates and strides.  
`get_buffer_region_from_load`(buffer_load[, extents]) | Get the buffer region from a buffer load.  
`get_extent`(data) | Return the inferred extent (shape) of a buffer-like object.  
  
## Module ContentsÂ¶

tilelang.language.utils.region(_buffer_ , _access_type_ , _* args_)Â¶
    

Create a tl.region call for a BufferLoad and extents.

Parameters:
    

  * **buffer** (_tvm.tir.BufferLoad_)

  * **access_type** (_str_)

  * **args** (_tvm.tir.PrimExpr_)



Return type:
    

tvm.tir.PrimExpr

tilelang.language.utils.buffer_region_to_tile_region(_buffer_region_ , _access_type_ , _extents_)Â¶
    

Clamp extents and return a tl.region call.

Parameters:
    

  * **buffer_region** (_tvm.tir.BufferRegion_)

  * **access_type** (_str_)

  * **extents** (_list_ _[__tvm.tir.PrimExpr_ _]_)



Return type:
    

tvm.tir.PrimExpr

tilelang.language.utils.index_to_coordinates(_index_ , _shape_)Â¶
    

Convert a flat (linear) index into multi-dimensional coordinates for a given shape.

Given a linear index and a shape (sequence of dimension extents), returns a list of coordinates (one per dimension) such that converting those coordinates back to a linear index using the usual row-major / C-order formula yields the original index. The computation iterates from the last dimension to the first using modulo and integer division, then reverses the collected coordinates.

Parameters:
    

  * **index** (_int_ _or_ _PrimExpr_) â The flat index to convert.

  * **shape** (_Sequence_ _[__int_ _]_) â The extents of each dimension (length >= 1).



Returns:
    

Coordinates for each dimension in the same order as shape.

Return type:
    

List[PrimExpr]

tilelang.language.utils.linear_index(_* args_)Â¶
    

Compute a flat (linear) index from multi-dimensional coordinates and strides.

The function accepts a sequence of PrimExpr arguments where the first portion are coordinates and the trailing portion are the corresponding strides. The number of strides must equal (number of coordinates - 1). The linear index is computed as:

> linear = coords[0] for each (coord, stride) in zip(coords[1:], strides):
>
>> linear = linear * stride + coord

Examples

  * linear_index(i) -> i

  * linear_index(i, j) -> i * j_stride + j (requires j_stride provided as stride when needed)

  * linear_index(i, j, stride_j) -> i * stride_j + j

  * linear_index(i, j, k, stride_j, stride_k) -> i*stride_j*stride_k + j*stride_k + k

  * linear_index(i, tx, v, threads, local_size) -> i*threads*local_size + tx*local_size + v




Raises:
    

**ValueError** â If called with no arguments, or if the number of strides is not one less than the number of coordinates.

Returns:
    

The computed linear index expression.

Return type:
    

PrimExpr

Parameters:
    

**args** (_tvm.tir.PrimExpr_)

tilelang.language.utils.get_buffer_region_from_load(_buffer_load_ , _extents =None_)Â¶
    

Get the buffer region from a buffer load.

May encounter buffer load like C[0:128, 0:32], ref to pull request for buffer wise op: <https://github.com/apache/tvm/pull/14693> convert load to region.

If the buffer load has ramp indices, we will use the rampâs base and lanes to create the region. Otherwise, return None since the load cannot be converted to a region.

Parameters:
    

  * **buffer_load** (_tvm.tir.BufferLoad_)

  * **extents** (_list_ _[__tvm.tir.PrimExpr_ _]__|__None_)



Return type:
    

tvm.tir.BufferRegion | None

tilelang.language.utils.get_extent(_data_)Â¶
    

Return the inferred extent (shape) of a buffer-like object.

If data is a Var bound to a let value, the let value is resolved before inspection.

Parameters:
    

**data** (_tilelang._typing.BufferLikeType_) â A Var, Buffer, BufferLoad or BufferRegion to inspect.

Returns:
    

The shape/extents as a list-like of PrimExpr (Buffer.shape or list of region item extents), or None if the extent cannot be determined.

Return type:
    

tilelang._typing.ShapeType | None
