# Tensor Checks (Host-Side Auto-Validation)Â¶

This page explains the host-side checks that TileLang automatically inserts into the generated host stub for kernels. When you pass `torch.Tensor` or any DLPack-compatible object to a TileLang kernel, the host stub validates argument count, pointer kinds, dtype, shape, strides, device, and more â so you donât need to handwrite Python checks. This keeps the ABI stable and significantly reduces Python overhead compared to doing equivalent checks in Python or via pybind.

## Why Host-Side ChecksÂ¶
  ...
  * Focused error reporting: assertions are raised close to the call site with precise âwhich field failedâ messages.




## How To Inspect Host SourceÂ¶
...
## What The Host ChecksÂ¶

### 1) Argument count and pointer kindÂ¶

  * `num_args` must match the number of formal parameters; otherwise the kernel returns `-1` with an error message.

  * Each argumentâs FFI type must be a pointer kind (for DLTensor/handle) or a valid scalar type; otherwise youâll see errors like `Expect arg[i] to be pointer` or a scalar type error.




### 2) Tensor checks (per tensor, after nullability decision)Â¶

  * Nullability

    * If the tensor is âstatically reachable/usedâ by the function body, the handle must be non-NULL; otherwise: `xxx is expected to have non-NULL pointer`.
  ...
  * Shape
    ...
    * Linear equations among symbolic dims can be solved on the fly (when thereâs only one unknown at a given check point), enabling cross-tensor constraints.
...
## Shapes and Symbolic Equations: Linear SolvingÂ¶
...
## Nullability Rules and ExamplesÂ¶
...
## Common Error Examples (What youâll see)Â¶
  ...
  * Shape constraint violation
    ...
    * Trigger: a dimension doesnât match a constant/symbol binding
...
## FAQÂ¶
  ...
  * Is the overhead noticeable?
    ...
    * The checks are lightweight (branches and field reads). Compared to Python-side checks, itâs faster; the dominating cost remains the PythonâC boundary. Overall itâs cheaper than equivalent checks in Python.
...
## Reference Example (Matmul + ReLU)Â¶
...
## Quick Error Reference (Short List)Â¶
...
## Host Error Troubleshooting (Minimal Repros)Â¶
...
### 0\. Tip: print the host sourceÂ¶
    
    
    print(fn.get_host_source())
    

### 1\. num_args mismatchÂ¶
...
### 2\. Expect pointer (tensor) but got scalarÂ¶
...
### 5\. Shape constant/symbol mismatchÂ¶
...
### 6\. Strides check failure (non-contiguous)Â¶
...
### 8\. device_id mismatch (multi-GPU)Â¶
...
### 9\. NULL data pointer (advanced)Â¶
...
### 10\. Scalar type mismatch (int / bool)Â¶
...
## Closing NotesÂ¶
  ...
  * Cross-check âshape / strides / device / dtypeâ against the kernel signature to localize issues efficiently.
