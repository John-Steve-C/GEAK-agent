# LetStmt Inlining in TileLangÂ¶

This document explains how `LetStmt` inlining works in TileLangâs simplification pipeline, which is an important optimization that affects code generation and performance.

## OverviewÂ¶

A `LetStmt` (Let Statement) is a temporary variable binding in the IR (Intermediate Representation). During compilation, TileLangâs simplifier may choose to inline these temporary variables to simplify the code. TileLang also provides a standalone `LetInline` pass that performs eager substitution before the main legalization pipeline. However, not all `LetStmt` nodes can be safely inlined.

## When Does LetStmt Get Inlined?Â¶
...
### 1\. The value satisfies `CanInlineLetStmt`Â¶
...
### 2\. The variable is NOT used in buffer definitionsÂ¶

Even if `CanInlineLetStmt` returns true, the variable will **not** be inlined if itâs used in a bufferâs definition (shape, strides, elem_offset, or data fields).
...
## Example: Why Buffer Definition Variables Are ProtectedÂ¶
...
Consider this code:
  ...
  * `stride` satisfies `CanInlineLetStmt` (itâs an int expression with no side effects)

  * However, `stride` is used in `buffer_a`âs `strides` field

  * If we inline it, the buffer definition becomes `strides=[M*16, 1]`

  * But the Buffer objectâs fields are not updated during simplification
...
## Practical Example: Temporary Variable IssueÂ¶
...
In this case:
  ...
  * Itâs **not** used in any buffer definition
...
## Controlling Let Inlining via Pass ConfigÂ¶
