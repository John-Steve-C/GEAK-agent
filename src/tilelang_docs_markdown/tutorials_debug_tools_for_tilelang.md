# Debugging Tile Language ProgramsÂ¶

_Author:_ [Lei Wang](https://github.com/LeiWang1999)

## OverviewÂ¶

A Tile Language program (hereafter referred to as a _program_) is transformed into a hardware-executable file through several stages:

  1. The user writes a Tile Language program.

  2. The program undergoes multiple _Passes_ for transformation and optimization (the _lower_ stage, see `tilelang/engine/lower.py`), finally producing an intermediate representation (e.g., LLVM or C for CPU, CUDA for NVIDIA GPUs, etc.).

  3. The generated code is compiled by the respective compiler (e.g., nvcc) into a hardware-executable file.


[![Overview of the compilation process](../_images/overview.png) ](../_images/overview.png)
...
This tutorial focuses on the first two issuesâhow to debug generation and correctness problems. Performance tuning often requires using vendor-provided profiling tools (e.g., **Nsight Compute** , **rocProf** , etc.) for further hardware-level analysis, which we will address in future materials.

Below, we take matrix multiplication (GEMM) as an example to demonstrate how to write and debug a Tile Language program.

## Matrix Multiplication ExampleÂ¶

In **Tile Language** , you can use the **Tile Library** to implement matrix multiplication. Hereâs a complete example:
...
## Debugging Generation IssuesÂ¶

TileLang essentially performs _progressive lowering_. For example, a `T.copy` may first be expanded into `T.Parallel` (see the pass `LowerTileOP`), which is then expanded again, eventually resulting in lower-level statements that can be translated to CUDA C code.

[![IR transformation diagram](../_images/ir_transform_diagram.png) ](../_images/ir_transform_diagram.png)
...
### Post-Processing Callbacks for Generated SourceÂ¶
...
### Runtime Debug Prints with `T.print`Â¶
...
### Visual Layout Inference For TileLangÂ¶
...
When enabled, `TL_LAYOUT_VISUALIZATION_FORMATS` accepts string values to control output formats:
  ...
  * âtxtâ: Text output only (same as default)

  * âallâ: Generates all formats (TXT, PDF, PNG, SVG)

  * âpngâ: Generate PNG format only

  * âpdfâ: Generate PDF format only

  * âsvgâ: Generate SVG format only

  * âtxt,svgâ: Generate multiple formats (comma-separated) in addition to text output




The output messages of âtxtâ will include something like:
...
## AutoDD: Automatic Delta DebuggingÂ¶
...
By carefully examining intermediate representations (IR) before final code generationâand by leveraging runtime printing through `T.print`âone can quickly diagnose where index calculations, copy logic, or other kernel operations deviate from the intended behavior. This two-pronged approach (inspecting IR transformations and using runtime prints) is often sufficient for resolving generation and correctness issues in TileLang programs.
