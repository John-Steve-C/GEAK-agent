# tilelang.jit.adapter.nvrtc.wrapperÂ¶
...
`TLNVRTCSourceWrapper` | NVRTC backend wrapper: generates Python kernel launch code.  
---|---  
  
## Module ContentsÂ¶

tilelang.jit.adapter.nvrtc.wrapper.PREDEF_HOST_FUNC_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.TMA_DESC_INIT_FUNC_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.TMA_IM2COL_DESC_INIT_FUNC_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.L2_PERSISTENT_MAP_CREATE_HANDLE_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.L2_PERSISTENT_MAP_INIT_FUNC_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.L2_PERSISTENT_MAP_RESET_HANDLE_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.PDL_SYNC_PY _ = Multiline-String_Â¶
    Show Value
...
tilelang.jit.adapter.nvrtc.wrapper.KERNEL_LAUNCH_FUNC_PY _ = Multiline-String_Â¶
    Show Value
...
_class _tilelang.jit.adapter.nvrtc.wrapper.TLNVRTCSourceWrapper(_scheduled_ir_module_ , _source_ , _target_ , _device_mod =None_, _host_mod =None_, _pass_configs =None_)Â¶
...
IRModule â collect kernel metadata â deduplicate resources â generate Python code â executable function
...
Parameters:
  ...
  * **code** â CUDA C++ source containing kernel declarations

  * **function_informations** â Dict mapping kernel names to metadata (grid/block dims, params, shared memory size)
...
**function_name** (_str_) â Kernel name to check for L2 persistence config
...
generate_pdl_sync_code(_function_name_)Â¶
...
Parameters:
  ...
  * **desc_name_map** (_dict_ _[__str_ _,__str_ _]_) â Maps descriptor variable names to buffer names

  * **desc_name_var_map** (_dict_ _[__str_ _,__tilelang.tvm.tir.Var_ _]_) â Maps descriptor names to TVM variables
...
**code** (_str_) â CUDA C++ source code containing compiled kernels
