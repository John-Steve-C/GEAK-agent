# General Matrix-Matrix Multiplication with Tile LibraryÂ¶

_Author:_ [Lei Wang](https://github.com/LeiWang1999)

Warning

This document is still **experimental** and may be incomplete. Suggestions and improvements are highly encouragedâplease submit a PR!
...
TileLang is a domain-specific language (DSL) designed for writing high-performance GPU kernels. It provides three main levels of abstraction:
  ...
  * **Level 2:** A user is aware of GPU architecture conceptsâsuch as shared memory, tiling, and thread blocksâbut does not necessarily want to drop down to the lowest level of explicit thread control. This mode is somewhat comparable to Tritonâs programming model, where you can write tile-level operations and let the compiler do layout inference, pipelining, etc.
...
![Parallel](../_images/Parallel.png)
  ...
  * `T.alloc_fragment` allocates register space for local accumulation. Though it is written as `(block_M, block_N)`, the compilerâs layout inference assigns slices of this space to each thread.
...
![Software Pipeline Inference](../_images/software_pipeline_inference.png)
...
TileLang Level 2 is conceptually similar to Triton in that the user can control tiling and parallelization, while letting the compiler handle many low-level details. However, TileLang also:
  ...
  * Enables mixing different levels in a single programâfor example, you can write some parts of your kernel in Level 3 (thread primitives) for fine-grained PTX/inline-assembly and keep the rest in Level 2.




## Performance on Different PlatformsÂ¶

![Performance on Different Platforms](../_images/op_benchmark_consistent_gemm_fp16.png)
...
These measurements will vary based on tile sizes, pipeline stages, and the hardwareâs capabilities.
...
For more advanced usageâincluding partial lowering, explicitly controlling thread primitives, or using inline assemblyâyou can explore Level 3. Meanwhile, for purely functional expressions and high-level scheduling auto-tuning, consider Level 1.
