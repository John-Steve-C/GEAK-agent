...
### With pip-provided CUDA toolchain (no host CUDA required)Â¶

If you donât have CUDA installed on the host, you can use pip-provided CUDA packages instead.

**Option A** â pip toolchain in the current environment (use `--no-build-isolation`):
...
**Option B** â pip toolchain in another virtualenv or path:
...
### Working from Source via `PYTHONPATH` (Recommended for Developers)Â¶
...
### Building with Customized TVM PathÂ¶
...
### ROCm container build (gfx942/gfx950)Â¶
...
> **Note:** Nightly builds contain the most recent code changes but may be less stable than official releases. Theyâre ideal for testing new features or if you need a specific bugfix that hasnât been released yet.

## Install ConfigsÂ¶

### Build-time environment variablesÂ¶

`USE_CUDA`: If to enable CUDA support, default: `ON` on Linux, set to `OFF` to build a CPU version. By default, weâll use `/usr/local/cuda` for building tilelang. Set `CUDAToolkit_ROOT` to use different cuda toolkit.
...
`NO_VERSION_LABEL` and `NO_TOOLCHAIN_VERSION`: When building tilelang, weâll try to embed SDK and version information into package version as below, where local version label could look like `<sdk>.git<git_hash>`. Set `NO_VERSION_LABEL=ON` to disable this behavior.
...
### Run-time environment variablesÂ¶
...
Building tilelang locally will automatically generate a `compile_commands.json` file in `build` dir. VSCode with clangd and [clangd extension](https://marketplace.visualstudio.com/items?itemName=llvm-vs-code-extensions.vscode-clangd) should be able to index that without extra configuration.
...
If you plan to use your wheel in other environment, itâs recommended to use auditwheel (on Linux) or delocate (on Darwin) to repair them.

### Faster Rebuild for DevelopersÂ¶
...
When running in editable/developer mode, youâll see logs like below:
