...
TileLang includes a builtâin autotuner that searches configuration spaces for the best performing kernel, compiles candidates in parallel, validates correctness, benchmarks them, and caches the best result for reuse.

This guide covers two workflows:

  * Decoratorâbased: `@tilelang.autotune(configs=...)` stacked on `@tilelang.jit`
...
Notes
  ...
  * `configs` can be a list of dicts or a callable `(args...) -> list[dict]`. Each dictâs keys must match the tunable function arguments (e.g., `block_M`).

  * The decorator returns a callable that runs autotune once per argument tuple and caches the resulting best kernel inâprocess.
...
### Example Gallery (in repo)Â¶

  * examples/gdn/example_chunk_delta_h.py:101 â uses `@autotune` to sweep configs

  * examples/deepseek_nsa/benchmark/benchmark_nsa_fwd.py:451 â uses `@tilelang.autotune`

  * examples/quickstart.py:84 â profiles a tuned kernel with `get_profiler`

  * examples/hadamard_transform/example_hadamard.py:152 â profiler with custom warmup

  * examples/dynamic_shape/example_dynamic.py:94 â profiler for dynamic shapes

  * examples/gemm/example_gemm_persistent.py:135 â compare persistent vs nonâpersistent
...
The tuner needs inputs to compile and benchmark kernels. Provide them in one of three ways (priority order):
  ...
  2. Custom supplier program
    ...
    def supply_prog(signature):
        ...
        # Return a list of torch tensors matching the kernelâs arguments
        return [A, B, C]
    
    tuner.set_profile_args(supply_prog=supply_prog)
    

  3. Builtâin generators via `supply_type`
...
Important
  ...
  * Builtâin generators require static shapes; if your PrimFunc uses symbolic dimensions (T.dyn), supply concrete inputs via (1) or (2).
...
## Correctness Checking and TolerancesÂ¶
...
## Configuration Spaces and Best PracticesÂ¶
...
Tips
  ...
  * Respect hardware limits (shared memory bytes, registers per thread/block, max threads per block). Eliminate impossible configs upâfront.
...
## Parallel Compilation/Benchmarking and TimeoutsÂ¶

The tuner compiles configurations in parallel using a thread pool and benchmarks them with a perâconfig timeout. On CUDA, each worker sets the current device to avoid context issues.

Notes

  * `timeout` uses POSIX signals; on nonâUnix systems, it may not take effect.
...
The autotuner caches best artifacts both inâmemory (per process) and on disk under `$TILELANG_CACHE_DIR/autotuner`. The cache key includes:

  * TileLang version, function source, closure freeâvars
...
Backend notes
  ...
  * Torch/DLPack backend may not save artifacts to disk; in this case, only inâmemory caching applies and a warning is logged.




## Alternative: Manual Sweeps with par_compileÂ¶
...
## Recording and Reusing Best ConfigsÂ¶

The programmatic path returns an `AutotuneResult` that can be saved and later reloaded. This is useful for CI, multiâhost workflows, or shipping tuned configs.
...
Notes
  ...
  * DLPack/Torch execution backend may not persist compiled binaries; in that case, reâcompilation is needed on load or use a different backend.

  * The directory contains humanâreadable JSONs (best config/latency) and sources.




## Advanced: Config Space CallablesÂ¶
...
## Device and Backend SelectionÂ¶

Tune compileâtime options explicitly:
...
## TroubleshootingÂ¶
  ...
  * âNo configurations to tuneâ: Ensure `configs` is a nonâempty list or callable.

  * Timeouts: Increase `timeout`; ensure inputs fit device memory; verify that your reference check isnât the bottleneck.
